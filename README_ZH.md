...
- **更适合阅读和审查**：树视图、JMESPath/JSONPath 查询、实时统计、JSON 对比，让复杂JSON数据更容易看懂。
...
![日志片段（深色）](docs/images_zh/log_fragment_dark.png)
...
通过树视图浏览复杂嵌套数据，一键复制路径和值；遇到大 JSON 时，也可以用 JMESPath 或 JSONPath 快速查询目标字段。
...
| 树视图与 JMESPath/JSONPath 查询 | 部分支持 | 支持 |
